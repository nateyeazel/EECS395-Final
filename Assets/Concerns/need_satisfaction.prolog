...
%% satisfies(addiction, $plant, 300, say("Ahh, that's the stuff"), 5).


need(socialCharizard, 60).
need(socialIvysaur, 60).
need(socialSquirtle, 60).

satisfies(socialCharizard, $charizard, 100, greet($me, $charizard), 5) :-
   $me \= $charizard.

satisfies(socialIvysaur, $ivysaur, 100, greet($me, $ivysaur), 5) :-
   $me \= $ivysaur.

satisfies(socialSquirtle, $squirtle, 100, greet($me, $squirtle), 5) :-
   $me \= $squirtle.
...
desirability_of_satisfying_need(Need, Delta, Desirability) :-
   ...
   Desirability is (Delta + Level) / (Level * distance($me, Object)).
%%min(Delta, 100-Level).
...
check_survival_needs :- \+dead($me),
                        survival_need(N),
                        satisfaction_level(N, X),
                        X =< 0,
                        kill($me).
check_survival_needs.

on_event(_, need_satisfaction, _, check_survival_needs).
...
increase_satisfaction(addiction, _).
